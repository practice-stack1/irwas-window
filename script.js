...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; //? сховище данних із модальних вікон
  ...
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div', 'after_click'); //! щоб добратись до необхідних блоків контенту ми викристали > div > div , так як контент був на 2 рівня нижче

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.balcon_icons', '.balcon_icons_img', '.big_img > img', 'do_image_more', 'inline-block');
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_3__["default"])(modalState);
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.container1', deadline);
  Object(_modules_images__WEBPACK_IMPORTED_MODULE_6__["default"])('.works', 'popup');
});
...
var changeModalState = function changeModalState(state) {
  ...
  var windowForm = document.querySelectorAll('.balcon_icons_img'),
      //? отримаємо таб із вікном необхідної форми
  windowWidth = document.querySelectorAll('#width'),
      windowHeight = document.querySelectorAll('#height'),
      windowType = document.querySelectorAll('#view_type'),
      windowProfile = document.querySelectorAll('.checkbox');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); //! збираємо дані із полів, які ми отримали вище
    ...
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          //! так як ми зчитуємо дані з усіх елементів, то читаємо ноду елемента, над яким проводимо дію і в залежності від типу витягуємо дані спец способом
          case 'SPAN':
            //? так як при виборі чекбокса ми звертаємось до спана, по контсрукції, то визначаємоь, до якого конкретно
            state[prop] = i;
            break;

          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              //? якщо це чекбокс, то виконуємо нижче
              i === 0 ? state[prop] = "Холодне" : state[prop] = "Тепле";
              elem.forEach(function (box, j) {
                //! виставляємо галочки лише для 1 елемента
                box.checked = false;
...
var forms = function forms(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); //! валідація форми на введення лише чисел
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); //! об'єкт для збору данних з полів (будь-яких типів)

      if (item.getAttribute('data-calc') === "end") {
        //? якщо форма з якою ми працюмо містить даний атребут(фінальна форма калькулятора), то... вносимо дані із стейта додатково в formData
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var images = function images(workfield, overlayStyle) {
  var imgPopup = document.createElement('div'),
      //? підлошка для майбутнього модального вікна
  workSection = document.querySelector(workfield),
      //? контейнер з фото
  bigImg = document.createElement('img');
  imgPopup.classList.add(overlayStyle); //? ставимо вже готовий стиль підстилки для модалки

  workSection.appendChild(imgPopup); //! встановлення стилів для нашого модального вікна

  imgPopup.style.justifyContent = 'center';
  imgPopup.style.alignItems = 'center';
  imgPopup.style.display = 'none';
  imgPopup.appendChild(bigImg); //! делігірумо головний контейнер
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('preview')) {
      //? перевіряємо чи даний елемент підтримує клік і чи клік відбувся по елементу із даним класом
      imgPopup.style.display = "flex";
      var path = target.parentNode.getAttribute('href'); //? беремо шлях до більшої картинки яка захована в батьківському елементі a

      bigImg.setAttribute('src', path);
    }

    if (target && target.matches('div.popup')) {
      //! якщо користувач клікнув на підстилку
      imgPopup.style.display = "none";
    }
  });
};
...
var modals = function modals() {
  function bindModals(triggerSelector, modalSelector, closeSelector) {
    ...
    modal.addEventListener('click', function (e) {
      if (e.target === modal && closeClickOverlay) {
        //! закриваємо лише при кліку на підошку чи хрестик
        window.forEach(function (item) {
          item.style.display = 'none';
        });
        modal.style.display = "none";
  ...
  bindModals('.phone_link', '.popup', '.popup .popup_close'); //! робота із вікнами, що замінятимуть одне одного
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  hideTabContent();
  showTabContent(); //! делігірування події кліку на таб контейнер

  header.addEventListener('click', function (e) {
    var target = e.target; //! отримуємо елемент на який безпосередньо створено клік

    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      //! перевіряємо чи містить даний таб необхідний нам клас (в селектора видалямо точку через регулярний вирах)
...
var timer = function timer(id, deadline) {
  ...
  var getTimeRemaining = function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        //? різниця між кінцевим часом і поточним
    seconds = Math.floor(t / 1000 % 60),
        //? отримамо к-сть секунд в секції таймера
    minutes = Math.floor(t / 1000 / 60 % 60),
        //? отримаємо к-сть хвилин
...
/* harmony import */ var slick_carousel__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(slick_carousel__WEBPACK_IMPORTED_MODULE_1__);
 //! імпортуємо наш jquery напряму з node-modules, без зайвого коду в html
